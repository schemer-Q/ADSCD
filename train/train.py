...
def main(config):
    ...
    if config["model_type"] == "gnm":
        ...
        # 在模型初始化后，定义 latent 参数
        z_dim = config.get("z_dim", 16)  # latent 向量维度
        p_drop = config.get("latent_drop_prob", 0.1)  # classifier-free 丢弃概率
        ...
        # 确保 NoMaD 支持 latent_z
        if config["model_type"] == "nomad":
            model = NoMaD(
                vision_encoder=vision_encoder,
                noise_pred_net=noise_pred_net,
                dist_pred_net=dist_pred_network,
                z_dim=z_dim  # 新增 latent 维度
            )
    ...
    if "pretrained_path" in config and config["pretrained_path"]:
        ...
        print(f"\nTry loading pretrained weights from {pretrained_path}\n")
        if os.path.exists(pretrained_path):
            ...
            if isinstance(ckpt, dict):
                if "state_dict" in ckpt:
                    ...
                    sd = ckpt  # 假设就是 state_dict
            else:
                sd = ckpt

            # 如果 checkpoint 的 key 带 "module." 前缀，但当前 model 未包装 DataParallel，则去掉前缀
            new_sd = {}
            for k, v in sd.items():
                new_k = k
            ...
            # 加载到模型（non-strict 可接受部分不匹配）
            load_result = model.load_state_dict(new_sd, strict=False)
            print("Pretrained load result:", load_result)
        else:
