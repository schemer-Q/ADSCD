...
def train_nomad(
    ...
    num_images_log: int = 8,
    use_wandb: bool = True,
    z_dim: int = 16,               # latent z 维度
    latent_drop_prob: float = 0.1, # classifier-free 丢弃概率
):
    """
    Train the model for one epoch.
    ...
    with tqdm.tqdm(dataloader, desc="Train Batch", leave=False) as tepoch:
        for i, data in enumerate(tepoch):
            ...
            B = actions.shape[0]
            # ---------------------------
            # 采样 latent_z + classifier-free
            # ---------------------------
            latent_z = torch.randn(B, z_dim, device=device)
            if np.random.rand() < latent_drop_prob:
                latent_z = None
    ...
    with tqdm.tqdm(
        ...
        for i, data in enumerate(tepoch):
            ...
            rand_mask_noise_pred = ema_model(
                ...
                latent_z=latent_z,        # NEW
                lambda_cfg=lambda_cfg,    # NEW
            )
