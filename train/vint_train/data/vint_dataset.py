...
# def _safe_load_image(path, transform=None, image_size=(96,96)):
#     """
#     尝试用 PIL 完整打开并 decode 图像；失败时打印路径并返回全零占位 tensor (C,H,W)。
#     transform: 若为 torchvision.transforms.Compose，直接传入 PIL.Image。
#     image_size: (H, W) 用于占位张量大小。
#     """
#     try:
...
#                     return transform(im)
#                 except Exception:
#                     # 某些 transform 期望 numpy array 或 tensor，尝试 ToTensor 再应用 normalize if present
#                     try:
...
#     except Exception:
#         # 记录失败路径并错误栈，输出到 stderr 避免被训练日志吞掉
...
#         # 1) 尝试从 LMDB 读取并用现有解码器处理
#         try:
#             with self._image_cache.begin() as txn:
#                 image_buffer = txn.get(image_path.encode())
#             if image_buffer is not None:
#                 image_bytes = io.BytesIO(bytes(image_buffer))
#                 out = img_path_to_data(image_bytes, self.image_size)
#                 # img_path_to_data 可能返回 numpy array 或 tensor
...
#             print(f"LMDB read/decode failed for {image_path}: {e}", file=sys.stderr)

#         # 2) 回退到文件系统，用 PIL 安全加载，确保返回 tensor
#         try:
#             tensor = _safe_load_image(image_path, transform=None, image_size=tuple(self.image_size))
...
#         except Exception as e:
#             print(f"Filesystem load failed for {image_path}: {e}", file=sys.stderr)
#             traceback.print_exc(limit=1)
...
#         # ensure yaw and positions are numeric arrays (some datasets store elements as 1D ndarrays)
#         yaw = np.asarray(yaw)
#         if yaw.dtype == object:
#             yaw = np.array([float(x) for x in yaw], dtype=float)
#         else:
...
#             except Exception:
#                 positions = np.array([[float(v) for v in p] for p in positions], dtype=float)
#         else:
#             positions = positions.astype(float)
#         goal_pos = traj_data["position"][min(goal_time, len(traj_data["position"]) - 1)]
#         # ensure goal_pos numeric vector
#         goal_pos = np.asarray(goal_pos)
#         if goal_pos.dtype == object:
#             try:
#                 goal_pos = np.array([float(x) for x in goal_pos], dtype=float)
#             except Exception:
...
#         obs_paths = [get_data_path(self.data_folder, f, t) for f, t in context]
#         obs_tensors = []
#         for p in obs_paths:
#             tensor = None
#             # 优先尝试从 LMDB 缓存读取并用现有解码器处理（与 _load_image 相同的逻辑）
#             try:
#                 with self._image_cache.begin() as txn:
#                     image_buffer = txn.get(p.encode())
#                 if image_buffer is not None:
#                     image_bytes = io.BytesIO(bytes(image_buffer))
#                     tensor = img_path_to_data(image_bytes, self.image_size)
#             except Exception:
#                 tensor = None

#             # 缓存或解码失败时，使用安全加载回退到 PIL（返回 torch.Tensor）
#             if tensor is None:
#                 tensor = _safe_load_image(p, transform=None, image_size=tuple(self.image_size))
#                 # 如果 img_path_to_data 返回 numpy array 的场景，把它转为 tensor
#                 if isinstance(tensor, np.ndarray):
#                     tensor = torch.as_tensor(tensor, dtype=torch.float32)

#             # 最终兜底：确保不是 None
#             if tensor is None:
#                 print(f"Warning: final fallback for {p}", file=sys.stderr)
#                 tensor = torch.zeros(3, self.image_size[0], self.image_size[1])

#             obs_tensors.append(tensor)
 
#         obs_image = torch.cat(obs_tensors, dim=0)
...
#         else:
#             #计算goal和current time之间应该存在的轨迹点数，waypoint_spacing理解为分辨率
...
#         #max_action_distance和min_action_distance都表示的是index而不是实际距离
#         action_mask = (
