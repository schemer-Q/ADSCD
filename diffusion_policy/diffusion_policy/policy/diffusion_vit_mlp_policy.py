...
from train.vint_train.models.vae.vit_mlp_encoder import ViTMLPEncoder

class DiffusionViTMLPPolicy(BaseLowdimPolicy):
    def __init__(self, 
            model: ConditionalUnet1D,
            noise_scheduler: DDPMScheduler,
            vit_encoder: ViTMLPEncoder,
            horizon, 
            obs_dim, 
            action_dim, 
            n_action_steps, 
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        """
        obs_dict: must include "obs" key and "goal_img" key
        ...
        # Generate latent z from visual inputs
        z_mean, z_logvar = self.vit_encoder(obs_img, goal_img)
        z = self.vit_encoder.sample(z_mean, z_logvar)
        ...
        local_cond = None
        global_cond = z  # Use latent z as global condition
    ...
    def compute_loss(self, batch):
        ...
        # Generate latent z distribution from visual inputs
        z_mean, z_logvar = self.vit_encoder(obs_img, goal_img)
        z = self.vit_encoder.sample(z_mean, z_logvar)
        ...
        # Total loss: diffusion loss + KL loss
        total_loss = diffusion_loss + self.kl_weight * kl_loss
