...
sys.path.append(os.path.join(project_root, 'diffusion_policy'))
# 添加train目录到Python路径
...
# 检查目录是否存在
print(f"ADSCD目录存在: {os.path.exists(project_root)}")
print(f"diffusion_policy目录存在: {os.path.exists(os.path.join(project_root, 'diffusion_policy'))}")
print(f"train目录存在: {os.path.exists(os.path.join(project_root, 'train'))}")
import torch
import hydra
